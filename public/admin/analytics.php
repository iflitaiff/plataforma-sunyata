...
$access_by_vertical = $db->queryAll("
    SELECT
        vertical,
        COUNT(*) as total_acessos,
        COUNT(DISTINCT user_id) as usuarios_unicos
...
$stats = [
    'total_users' => $db->query("SELECT COUNT(*) as count FROM users")['count'] ?? 0,
    'users_completed_onboarding' => $db->query("SELECT COUNT(*) as count FROM users WHERE completed_onboarding = TRUE")['count'] ?? 0,
    'total_tool_accesses' => $db->query("SELECT COUNT(*) as count FROM tool_access_logs")['count'] ?? 0,
    'total_accesses_30d' => $db->query("SELECT COUNT(*) as count FROM tool_access_logs WHERE accessed_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)")['count'] ?? 0
];
